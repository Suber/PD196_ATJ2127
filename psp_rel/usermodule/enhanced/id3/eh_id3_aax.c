...
bool get_id3_aax_sub(uint32 end_pos_param)
{
    ...
    while (id3_pos_file < end_pos_param)
    {
        ...
        if (result != (int8) - 1)
        {
            ...
            {
                str_length = aax_get_frame_size() - 8;
                num++;
                if (id3_save_p->savelen != 0)
                {
                    ...
                    if (key_buf[3] == 1)
                    {
                        ...
                        str_length = id3_read_utf8_deal(id3_save_p, str_length, NULL);

                        //                        if (str_length > ((uint32) id3_save_p->savelen - 4))
                        //                        {
                        //                            str_length = id3_save_p->savelen - 4;
                        //                        }
                        //
                        //                        read_buf_data(id3_save_p->saveaddr + 3, str_length);
                        //                        *(id3_save_p->saveaddr + str_length + 3) = 0x00;
                    ...
                    else if (key_buf[3] == 2)
                    {
                        ...
                        *(uint16*) (id3_save_p->saveaddr) = UNICODE_FLAG;
                        read_buf_data(id3_save_p->saveaddr + 2, str_length);
                        *(uint16*) (id3_save_p->saveaddr + (str_length&0xFFFE) + 2) = 0x00;
                    }
                    else
                    {
                        //asc
                        if (str_length > ((uint32) id3_save_p->savelen - 1))
                        {
                            str_length = id3_save_p->savelen - 1;
                        }
                        read_buf_data(id3_save_p->saveaddr, str_length);
                        *(id3_save_p->saveaddr + str_length) = 0x00;
                    }

                    frame_size = frame_size - 8 - str_length;
                }
        ...
        else if ((1 == id3_info_p->apic_flag) && (0 == libc_memcmp(&key_buf[4], pic_string, 4)))
        {
            ...
            if (result != -1)
            {
                id3_info_p->apic_offset = id3_pos_file + result - 64;
