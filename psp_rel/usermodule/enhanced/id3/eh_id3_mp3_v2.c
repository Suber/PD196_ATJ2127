...
bool mp3_v2_2_parse(uint32 id3_size_param)
{
    ...
    while ((id3_pos_file < id3_size_param) && (num < ID3_COUNT1))
    {
        ...
        frame_len = frame_h->size[0] * 0x10000 + frame_h->size[1] * 0x100 + frame_h->size[2];
        ...
        {
            result = mp3_check_tag(frame_h->frame_id, (uint8*) &id3_tab_v2_2, 3, ID3_COUNT1);
            if (result != (int8) - 1)
            {
                num++;
                if (id3_save_p->savelen != 0)
                {
                    ...
                    {//utf8
                        ...
                        str_len = id3_read_utf8_deal(id3_save_p, str_len, NULL);
...
bool mp3_v2_x_parse(uint32 id3_size_param, uint8 version)
{
    ...
    while ((id3_pos_file < id3_size_param) && (num < ID3_COUNT2))
    {
        ...
        if (frame_len == 0)
        {
            if (*(uint32*) frame_h->frame_id == 0x00)
            {//padding data
                break;
            }
            //内容为空，此情况比较特殊
...
//                        *(uint8*) (id3_save_p->saveaddr + 3 + str_len) = 0x00;
        ...
        else if ((frame_h->frame_id[0] == 'A') && (id3_info_p->apic_flag))
        {
            ...
            if (result != (int8) - 1)
            {
                ...
                if (result != -1)
                {
                    id3_info_p->apic_offset = id3_pos_file - 64 + result;
...
bool mp3_v2_parse(void)
{
    ...
    //ID3Size是整个ID3V2的长度
    id3_size = sizeof(id3_10_header) + (id3h->size[0] & 0x7F) * 0x200000 + (id3h->size[1] & 0x7F) * 0x4000
