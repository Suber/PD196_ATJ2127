...
static void check_id3_is_empty(id3_info_t *id3_info, id3_type_e file_type);
static void filtrate_char_sub(uint8 *buf_pt);
 ...
 * \note
 *******************************************************************************/
static void filtrate_id3_char(id3_info_t *id3_info)
{
    uint32 *buf_pt;
    uint8 *len_pt;
    uint8 count = 6;
...
static void check_id3_is_empty(id3_info_t *id3_info, id3_type_e file_type)
{
    ...
    {
        vfs_get_name(vfs_mount, id3_info->tit2_buffer, id3_info->tit2_length / 2);
...
static void find_album_jpg(id3_info_t *id3_info)
{
    uint8 result = 1;
    //save current path
    vfs_file_dir_offset(vfs_mount, bank_temp_buf, bank_temp_buf + 76, 0);
    ...
    if (result == 1)
    {
        ...
        vfs_file_dir_offset(vfs_mount, NULL, (uint8*) id3_info ->apic_offset, 0);
        id3_info ->apic_type = 2;//set album jpg flag
    }

    // recover path
    vfs_file_dir_offset(vfs_mount, bank_temp_buf, bank_temp_buf + 76, 1);
