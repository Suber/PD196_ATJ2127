...
{ 0x57, 0x00, 0x4D, 0x00, 0x2F, 0x00, 0x41, 0x00, 0x6C, 0x00, 0x62, 0x00, 0x75, 0x00, 0x6D, 0x00, 0x54, 0x00, 0x69,
        0x00, 0x74, 0x00, 0x6C, 0x00, 0x65, 0x00 };
//  WM/Genre 的unicode
...
{ 0x57, 0x00, 0x4D, 0x00, 0x2F, 0x00, 0x54, 0x00, 0x72, 0x00, 0x61, 0x00, 0x63, 0x00, 0x6B, 0x00, 0x4E, 0x00, 0x75,
        0x00, 0x6D, 0x00, 0x62, 0x00, 0x65, 0x00, 0x72, 0x00 };
//  WM/Picture 的unicode
static const uint8 picture_descriptor_name[20] =
{ 0x57, 0x00, 0x4D, 0x00, 0x2F, 0x00, 0x50, 0x00, 0x69, 0x00, 0x63, 0x00, 0x74, 0x00, 0x75, 0x00, 0x72, 0x00, 0x65,
        0x00 };
...
static bool wma_excontent_object(uint32 objsize)
{
    ...
    while ((id3_pos_file < objsize) && (N < count))
    {
        ...
        else
        {
            ...
            else if ((result == 6) && (id3_info_p->apic_flag))
            {
            ...
            else
            {
                ...
                if (value_type == 0)
                {
                    ...
                    *(uint16*) (id3_save_p->saveaddr + (namelen&0xFFFE) + 2) = 0x00;//这里补字符串结束注意结尾应该是两个0
