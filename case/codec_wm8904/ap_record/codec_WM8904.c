...
{ 8000, 8000, 16000, 32000, 32000, 16000, 16000, 24000, 32000, 48000, 96000 };
*/
static const uint16 codec_rate_set[] = 
{0xc00,0xc00,0xc02,0xc04,0xc04,0xc02,0xc02,0xc03,0xc04,0xc05,0x405};


UINT8  codec_i2s_sn = 0xff;
UINT8  cur_bitrat_set;
UINT32 mfp_ctlx_bak;
...
void Enable_i2s(uint8 sn)
{
    ...
    else
    {
        ...
        tmp32 = mfp_ctlx_bak & 0xffffe000; 
        act_writel((tmp32|0x1b35), MFP_CTL3);   //I2S GPIOB 15 14 13 12
    } 
}
...
//    act_writel((act_readl(MFP_CTL2) & 0xfff8dfff), MFP_CTL2);   //0xC0090034
//    act_writel((act_readl(CLKENCTL) | 0x07000), CLKENCTL);//0xc0000008
//    act_writel((act_readl(MRCR) | 0x000c00), MRCR);//0xc0000000
//    act_writel(0x0c201, DAC_CTL);//0xc0050000
//    act_writel((act_readl( AUDIOPLL_CTL) | 0x0902),  AUDIOPLL_CTL);//0xc000010c  
//    act_writel((act_readl(ADC_FIFOCTL) | 0x080b), ADC_FIFOCTL);//0xc005100c          
...
int Codec_set_brat(uint8 bitrate_type)
{
    ...
    if(cur_bitrat_set != bitrate_type)
    {
        cur_bitrat_set = bitrate_type;
    }
    else
    {
        return 0;
    ...
    tmp16 = codec_rate_set[bitrate_type];    
